...
RUN \
  ...
  repo=$(docker-helper git_clone_ruby_repo "jekyll@4.3.2") && \
  if [ ! -z "$repo" ]; \
  then \
    cd $repo && \
    ...
    yes | docker-helper ruby_install_gem \
      "jekyll@4.3.2" --no-document -- \
        --use-system-libraries; \
  fi && \

  cd ~ && \
  docker-helper install_default_gems && \
  gem clean && gem install bundler -v '= 2.3.26' --no-document && \
